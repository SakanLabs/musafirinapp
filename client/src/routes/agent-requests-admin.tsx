...
export const Route = createFileRoute("/agent-requests-admin")({
  beforeLoad: async () => {
    const isAuth = await authService.isAuthenticated();
    if (!isAuth) throw new Error("Unauthorized");
  },
  component: AdminAgentRequestsPage,
});
...
const statusConfig: Record<string, { label: string; class: string }> = {
  ...
  quoted: { label: "Penawaran", class: "bg-purple-50 text-purple-700 border-purple-200/50" },
  ...
  payment_uploaded: { label: "Verifikasi Bayar", class: "bg-yellow-50 text-yellow-700 border-yellow-200/50" },
...
const statusOptions = [
  ...
  { value: "submitted", label: "Terkirim (Baru)" },
  { value: "in_review", label: "Sedang Diproses" },
  { value: "quoted", label: "Menunggu Respon Agent" },
  { value: "quote_revision_requested", label: "Revisi Penawaran" },
  { value: "quote_accepted", label: "Penawaran Diterima" },
  { value: "payment_uploaded", label: "Menunggu Verifikasi Pembayaran" },
  { value: "paid", label: "Terbayar" },
  { value: "completed", label: "Selesai" },
];

function AdminAgentRequestsPage() {
  const navigate = useNavigate();
  const [requests, setRequests] = useState<AdminAgentRequest[]>([]);
  ...
  const loadRequests = async () => {
    try {
      ...
      const res = await apiClient.get<any>(`/api/agent-requests/admin/all${qs}`);
      if (res.success) {
        // Filter out drafts since admin shouldn't normally see them unless requested
        const nonDrafts = res.data.filter((r: any) => r.status !== 'draft');
  ...
  const filtered = requests.filter((req) => {
    ...
    return (
      ...
      (req.agentName || "").toLowerCase().includes(s) ||
      (req.agentEmail || "").toLowerCase().includes(s)
    );
  });
  ...
  return (
    <PageLayout
      ...
      subtitle="Kelola permintaan layanan dari agent / reseller"
    >
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Cari nomor request, agent, atau judul..."
              value={search}
                    ...
                    <tr key={req.id} className="hover:bg-[#f8f9fa] transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-col">
                          <span className="text-xs font-semibold text-[#111111]">{req.agentName || "Unknown"}</span>
                          <span className="text-[10px] text-gray-500">{req.agentEmail}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-center">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => navigate({ to: "/agent-request-admin-detail/$id", params: { id: req.id.toString() } })}
